...
func Web() {
	...
	for {
		...
		if lc {
			...
			for i, v := range distances{ // loop through distances
				if v > float64(tension){ // only pull if over tension
					...
					relativeX := basesX[i] - cx; relativeY := basesY[i] - cy
			...
			if math.Abs(centerVelX) < 0.1 { centerVelX = 0 }
			if math.Abs(centerVelY) < 0.1 { centerVelY = 0 }
		}
		...
		// draw circles
		troglodyte.GlobalCircleRatio = 2.5
